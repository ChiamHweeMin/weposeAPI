...
app.post('/WEPOSE/initSitPosture', async (req, res) => {
	try {
		...
		const pythonScript1 = spawn('/usr/bin/python', ['./ModelTraining.py', JSON.stringify(data)]);
		...
		const pythonScript2 = spawn('/usr/local/bin/python', ['./ModelPrediction.py', JSON.stringify(modelData), JSON.stringify(newSample)]);
